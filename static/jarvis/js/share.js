  ...
  $(document).ready(function() {
    ...
    var loadShare = function(e) {
      ...
      $.ajax({
        ...
        if(res.recode == 0) {
          var template = '<form id="addForm"><input type="hidden" name="numid" value="{{ numid }}"/><input type="hidden" name="url" value="{{ url }}"/><input type="hidden" name="brand" value="{{ brand }}"/><input type="hidden" name="title" value="{{ title }}"/><input type="hidden" name="pic" value="{{ pic }}"/><input type="hidden" name="price" value="{{ price }}"/><div class="share-pic clearfix"><img src="{{ pic }}"></div><div class="share-content clearfix"><p><span>¥{{ price }}</span><span title="{{ title }}">{{ title }}</span></p><p><span>请选择分享的类别：</span><select name="type"><option value="1">狗粮</option><option value="2">狗窝</option><option value="3">狗链</option><option value="4">狗笼子</option><option value="5">狗狗衣服</option></select></p><div><p><span id="alreadyShare" class="hide">已经分享过了哦~<a href="javascript:void(0);" id="switchShare">换一个分享</a></span><span class="float-right">还可以输入<span id="pubLeft">140</span>个字</span></p><div class="twitter-wrap"><textarea id="pubContent" name="twitter" placeholder="说点什么吧~" required="required"></textarea><button type="submit" class="pub-button">发表</button></div></div></div></form>',
            context = res.share;
